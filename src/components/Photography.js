import React from 'react'
import StarOutlineIcon from '@mui/icons-material/StarOutline';

function Photography() {
  return (
    <div className="photographyyy">
    <div className='photowidth'>
      <img className='photgraphyimage' src="https://media.istockphoto.com/id/1427212392/photo/production-team-shooting-some-video-movie-for-tv-commercial-with-studio-equipment-set.webp?b=1&s=170667a&w=0&k=20&c=AWux21qfu1KMdFk90M43HZXEcSSRCEHUNKew1ifmjNU=" />
      <div>
      <span className='iconstarss'>4.7 <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span > <StarOutlineIcon /> </span>
      <span > <StarOutlineIcon /> </span>
      </div>
     <h1>services we offers</h1>
    </div>
    <div className='photowidth'>
      <img className='photgraphyimage' src="https://media.istockphoto.com/id/1139957268/photo/group-of-friends-celebrating-birthday-together.webp?b=1&s=170667a&w=0&k=20&c=GO59q8S7ui1C4rcVhgYfrrK3Pqaci4IDsaFG5l0NAuM=" />
      <span className='iconstarss'> 5.00<StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
     <h1>services photorapher</h1>
    </div>
    <div className='photowidth'>
      <img className='photgraphyimage' src ="https://images.unsplash.com/photo-1632187989763-c9c620420b4d?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8dmlkZW8lMjBwcm9kdWN0aW9ufGVufDB8fDB8fHww" />
      <span className='iconstarss'> 4.8<StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon /> </span>
      <span className='iconstarss'> <StarOutlineIcon  /> </span>
     <h1>docuration goods</h1>
    </div>
    
   </div>
  )
}

export default Photography